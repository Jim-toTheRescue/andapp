...
class MainActivity : AppCompatActivity() {
    ...
    private fun getCurrentVersion(): String {
        return try {
            ...
            FileServerService.addLog("packageName=$packageName")
            FileServerService.addLog("versionName=${packageInfo.versionName}")
    ...
    private fun fetchLatestRelease(): Triple<String, String, String>? {
        return try {
            ...
            if (responseCode == 200) {
                ...
                // 从 Release name 提取版本号，格式如 "瓦力 v1.0.82"
                val name = json.optString("name", "")
                val versionMatch = """v?(\d+\.\d+\.\d+)""".toRegex().find(name)
                val tagName = versionMatch?.groupValues?.getOrNull(1) ?: "0"
